...
def parse_args():
    ...
    parser.add_argument(
        "--model", type=str, default="pointnet2_feature_seg_ssg", help="model name"
    )
    parser.add_argument(
    ...
    parser.add_argument("--seg_mode", type=str, default="mat", help="seg_mode")
...
def main(args):
    ...
    exp_dir = exp_dir.joinpath("feature_{}_seg".format(args.seg_mode))
    exp_dir.mkdir(exist_ok=True)
    ...
    TRAIN_DATASET = CompatSeg(
        data_root=root, num_points=args.npoint, split="train", transform=None, seg_mode=args.seg_mode, use_features=True
    )
    VAL_DATASET = CompatSeg(
        data_root=root, num_points=args.npoint, split="valid", transform=None, seg_mode=args.seg_mode, use_features=True
    )
    ...
    num_partmat = num_mat* num_part + num_mat
    num_features = num_part if args.seg_mode == "part" else num_mat if args.seg_mode == "mat" else None
    ...
    if args.model == "curvenet_seg" or args.model == "pointmlp":
        classifier = MODEL.get_model(
            num_features, shape_prior=shape_prior, npoints=args.npoint, seg_mode=args.seg_mode
        ).cuda()
    else:
        classifier = MODEL.get_model(
            num_features, shape_prior=shape_prior, normal_channel=args.normal, seg_mode=args.seg_mode
        ).cuda()
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for _, (points, label, target, _) in tqdm(
            ...
            seg_pred = seg_pred.contiguous().view(-1, num_features)
        ...
        with torch.no_grad():
            ...
            for _, (points, label, target, _) in tqdm(
                ...
                for partk_k in range(num_features):
                ...
                miou = compute_overall_iou(cur_pred_val, target, num_features)
