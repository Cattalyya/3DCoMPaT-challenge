...
def parse_args():
    ...
    parser.add_argument(
        "--model", type=str, default="pointnet2_partmat_seg_ssg", help="model name"
    )
    parser.add_argument(
...
def main(args):
    ...
    TRAIN_DATASET = CompatSeg(
        data_root=root, num_points=args.npoint, split="train", transform=None, seg_mode="partmat",
    )
    VAL_DATASET = CompatSeg(
        data_root=root, num_points=args.npoint, split="valid", transform=None, seg_mode="partmat",
    )
    ...
    if args.model == "curvenet_seg" or args.model == "pointmlp":
        classifier = MODEL.get_model(
            num_partmat, shape_prior=shape_prior, npoints=args.npoint, seg_mode="partmat"
        ).cuda()
    else:
        classifier = MODEL.get_model(
            num_partmat, shape_prior=shape_prior, normal_channel=args.normal, seg_mode="partmat"
        ).cuda()
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for _, (points, label, target, _) in tqdm(
            ...
            seg_pred = seg_pred.contiguous().view(-1, num_partmat)
        ...
        with torch.no_grad():
            ...
            total_correct = 0
            total_seen = 0
            total_seen_class = [0 for _ in range(num_partmat)]
            total_correct_class = [0 for _ in range(num_partmat)]
            ...
            for _, (points, label, target, _) in tqdm(
                ...
                for partk_k in range(num_partmat):
                ...
                miou = compute_overall_iou(cur_pred_val, target, num_partmat)
